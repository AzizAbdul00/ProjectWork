...
<body class="bg-white">
    ...
    <div class="mx-auto max-w-screen-xl px-4 py-28 sm:px-6 lg:px-8">
        ...
        <form action="{{ route('register-auth') }}" class="mx-auto mb-0 mt-8 max-w-md space-y-4" method="POST">
            @csrf
            <div>
                <label for="name" class="sr-only">Username</label>
                <div class="relative">
                    <input type="text"
                        class="w-full rounded-lg border-gray-200 p-4 pe-12 text-sm shadow-sm border focus:border-gray-400 focus:outline-none"
                        placeholder="Enter name" name="name" id="name" value="{{ old('name') }}" />
                    <span class="absolute inset-y-0 end-0 grid
